...
export default function AddNewCar() {
      ...
      try {
        ...
        if (gallery.length === acceptedFiles.length) {
          const {
            ...
            fuelType,
            transmissionType,
          } = brands[selctedBrand].models[selctedModel];
          mutation.mutate({
            ...formData,
            gallery,
            brand: brands[selctedBrand].name,
            ...{ model, modelYears, fuelType, transmissionType },
          });
        } else {
  ...
  return (
    <>
      <Pagetitle> Add New Car || DriveHub</Pagetitle>
      <form ref={formref} onSubmit={handelData} className="p-10">
        ...
        <div className="grid lg:grid-cols-2 gap-5 mt-5">
          <div className="form-control ">
            ...
            <select
              onChange={(e) => (
                setSelectedBrand(e.target.value), setSelectedModel("")
              )}
              required
              className="select select-bordered w-full bg-transparent border-black text-black text-lg"
            >
              <option value="">Select a Brand</option>
              {brands.length > 0 &&
                brands.map((ele, ind) => {
                  return (
                    <option value={ind} key={ele.id}>
                      {ele.name}
                    </option>
                  );
                })}
          ...
          {selctedBrand && (
            <div className="form-control ">
              <label className="label">
                {" "}
                <span className="label-text text-lg font-bold">Model</span>{" "}
              </label>
              <select
          ...
          {selctedModel && (
            <Input
              label={"Model Years"}
              type={"text"}
              placeholder={"Set Per Day Price"}
              value={brands[selctedBrand].models[selctedModel].modelYears}
              readOnly
            ></Input>
          )}
          ...
          {selctedModel && (
            <Input
              ...
              type={"text"}
              placeholder={"Set Per Day Price"}
              value={brands[selctedBrand].models[selctedModel].fuelType}
              readOnly
            ></Input>
          )}
          ...
          {selctedModel && (
            <Input
              ...
              type={"text"}
              placeholder={"Set Per Day Price"}
              value={brands[selctedBrand].models[selctedModel].transmissionType}
              readOnly
            ></Input>
          )}
          ...
          <div className="col-span-2">
            <button className="btn w-full bg-red-600 text-white hover:bg-red-600 mt-5">
              Add now
            </button>
          </div>
        </div>
